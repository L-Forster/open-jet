...
class BenchmarkTests(unittest.TestCase):
    ...
    def test_run_benchmark_uses_mtp_for_qwen38_without_config_flag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            model = Path(tmp) / "Qwen3.8-27B-Q4_K_M.gguf"
            model.write_bytes(b"GGUF")
            cfg = {
        ...
        run_mtp.assert_called_once()
        run_bench.assert_not_called()

    def test_resolve_device_upgrades_stale_vulkan_when_cuda_is_available(self) -> None:
        with patch("src.benchmark.recommended_device", return_value="cuda"):
            self.assertEqual(benchmark._resolve_device({"device": "vulkan"}), "cuda")

    def test_resolve_device_keeps_vulkan_when_binary_is_vulkan_only(self) -> None:
        with patch("src.benchmark.recommended_device", return_value="cuda"), patch(
            "src.benchmark._binary_is_vulkan_only", return_value=True
        ):
            self.assertEqual(
                benchmark._resolve_device(
                    {"device": "vulkan", "llama_server_path": "/opt/openjet/bin/llama-server"}
                ),
                "vulkan",
            )
