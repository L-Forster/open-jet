...
## 1) Load a runtime and run inference
...
During setup, choose your runtime/model profile. After setup, OpenJet starts the selected backend and keeps the agent session bounded.

### Run inference in the chat UI

Type a normal prompt, for example:

```text
Summarize the latest logs and propose next debugging steps.
```
